...
export default function EditProjectScreen() {
  ...
  const { data: projectDataRes, isLoading: loadingProject } = useQuery({ 
  ...
  useEffect(() => {
    if (projectDataRes?.project) {
      const p = projectDataRes.project;
      setForm({
        title: p.title || '',
        client: p.client || '',
        department_id: p.department_id ? p.department_id.toString() : '',
        assigned_to: p.assigned_to ? p.assigned_to.toString() : '',
  ...
  const mutation = useMutation({
    ...
    onError: (err: any) => {
      setGlobalError(err.response?.data?.message || 'Failed to update project');
    }
  });
      ...
      <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <Card style={styles.card}>
          ...
          <View style={styles.actionRow}>
            ...
            <TouchableOpacity style={styles.cancelBtn} onPress={() => router.replace(`/(drawer)/projects/${projectId}`)}>
