...
**Update (same day, follow-up request):** the user asked to fix the duplicate-naming problem and add edit/delete. This upgraded the feature to exactly the "full client management" design originally scoped out:
- New `clients` table (`App\Models\Client`) and a nullable `projects.client_id` foreign key (`nullOnDelete`). Migration `2026_07_24_000002_add_client_id_to_projects_table` backfills it by normalizing existing `projects.client` strings — case/whitespace variants like "Acme Corp" vs "acme corp " are merged into one canonical `clients` row, which is the actual fix for the duplicate-client problem. **Not executed in this sandbox (no PHP) — must run `php artisan migrate --force` on a real machine before this feature works; see RELEASE_CHECKLIST.md.**
- `Client::resolveForProject()` — used by both `ProjectController` (store/update) and `SyncService` (offline `create_project`) — find-or-creates a Client from a `client_id` or a raw `client` string, so the project create/edit forms didn't need to change at all; typing a name that case-insensitively matches an existing client reuses it instead of creating a duplicate, going forward.
- `projects.client` (the string column) is kept as a denormalized cache of the linked Client's name — renaming a client via `PUT /api/clients/{id}` cascades the new name onto every linked project's `client` column, since billing, invoices, and the project detail screens all still read that column directly.
- `DELETE /api/clients/{id}` returns 409 if any project still references the client (`$client->projects()->exists()`), per explicit request ("stop delete if its linked to any project"). Founder-only for create/rename/delete; view access unchanged (founder/head/accounts).
- `clients.tsx` gained an "Add Client" button, a per-row edit (rename) action, and a per-row delete action (blocked-delete surfaces the 409 message via `window.alert`/`alert`).
- `backend/tests/Feature/ClientControllerTest.php` extended with 5 more cases covering create, case-insensitive duplicate rejection, non-founder rejection, rename-cascades, delete-blocked, and delete-succeeds-when-unlinked. `ProjectControllerTest.php` gained 2 more cases (client_id gets set on create; a second project with a differently-cased client name reuses the same Client). **None of the PHPUnit tests were executed — PHP is still unavailable in this sandbox.**
- Two new Playwright E2E tests (rename flow, blocked-vs-successful delete flow) **were** executed against the real production web export with a real headless Chromium — both pass.
